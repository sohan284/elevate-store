      ...
      <aside className={cn(
        ...
        <div className="flex flex-col h-full">
            ...
            {sidebarItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  href={item.href}
                  className={cn(
                    "flex items-center gap-4 px-5 py-3.5 rounded-xl transition-all duration-200 group text-[14.5px] font-medium",
                    isActive 
                      ? "bg-primary text-white shadow-lg shadow-primary/30" 
                      : "text-gray-400 hover:bg-white/5 hover:text-white"
                  )}
                >
                  <item.icon size={20} className={cn(isActive ? "text-white" : "text-gray-500 group-hover:text-primary transition-colors")} />
                  {item.name}
          ...
          <div className="p-6 border-t border-white/5 bg-black/10">
            <button className="flex items-center gap-4 px-5 py-3 w-full text-gray-400 hover:text-red-400 transition-colors rounded-xl hover:bg-red-500/10">
              <LogOut size={18} />
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="h-20 bg-white border-b border-gray-100 flex items-center justify-between px-6 md:px-10 shrink-0 sticky top-0 z-30">
          ...
          <div className="flex items-center gap-4 sm:gap-6">
            {/* Notification Badge */}
            <button className="relative p-2.5 rounded-xl text-gray-500 hover:bg-gray-50 hover:text-primary transition-all">
              <Bell size={22} />
            ...
            <div className="flex items-center gap-3.5 pl-4 sm:pl-6 border-l border-gray-100 cursor-pointer group">
              ...
              <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-primary to-blue-700 flex items-center justify-center text-white font-bold text-sm shadow-lg shadow-primary/20 group-hover:scale-105 transition-transform">
                MS
              </div>
            </div>
          </div>
