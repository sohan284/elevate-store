...
export default function AdminLayout({
  ...
  // 🛡️ Expert Security: Only allow admins to see this layout
  // useEffect(() => {
  //   if (!isAuthenticated || user?.role !== "admin") {
  //     router.replace("/signin?message=Access Denied. Admin only.");
  //   }
  ...
  return (
    <div className="flex h-screen bg-[#F8F9FA] overflow-hidden">
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <div className="p-6 border-t border-white/5 bg-black/10">
            <button
              onClick={() => logout()}
              className="flex items-center gap-4 px-5 py-3 w-full text-gray-400 hover:text-red-400 transition-colors rounded-lg hover:bg-red-800/10"
            >
              <LogOut size={18} />
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {/* Top Header */}
        <header className="h-20 bg-white border-b border-gray-100 flex items-center justify-between p-5 md:p-6 lg:p-8 shrink-0 sticky top-0 z-30">
          ...
          <div className="flex items-center gap-4 sm:gap-6">
            ...
            <div className="flex items-center gap-3.5 pl-4 sm:pl-6 border-l border-gray-100 cursor-pointer group">
              <div className="hidden sm:block text-right">
                <p className="text-[14px] font-bold text-gray-900 leading-none mb-1 truncate max-w-[120px]">
                  {user?.displayName}
                </p>
                <p className="text-[11.5px] font-semibold text-primary/80 uppercase tracking-wider">
                  {user?.role} Admin
                </p>
              </div>
              <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-primary to-blue-700 flex items-center justify-center text-white font-bold text-sm shadow-lg shadow-primary/20 group-hover:scale-105 transition-transform overflow-hidden">
                {user?.avatar ? (
                  <img
                    src={user?.avatar}
                    alt={user?.firstName}
        ...
        <div className="flex-1 min-h-0 p-5 md:p-6 lg:p-8 bg-[#FBFBFC] overflow-y-auto">
          {children}
        </div>
      </main>
