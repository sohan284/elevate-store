...
import { Menu, X, ChevronRight, User, Contact, Heart, HelpCircle, FileText } from "lucide-react";
...
export default function MobileMenu() {
  ...
  const categories = [
    { name: "Honey", hasIcon: true },
    { name: "Dates", hasIcon: true, active: true },
    { name: "Spices", hasIcon: true },
    { name: "Nuts & Seeds", hasIcon: true },
    { name: "Beverage", hasIcon: true },
    { name: "Rice", hasIcon: false },
    { name: "Flours & Lentils", hasIcon: true },
    { name: "Certified", hasIcon: false },
    { name: "Pickle", hasIcon: false },
  ];
        ...
        <div className="p-4 pt-4 flex-1 pb-10">
          ...
          <div className="bg-primary text-white p-4 rounded-xl flex items-center gap-3 mb-6 relative overflow-hidden">
            ...
            <div className="flex flex-col relative z-10">
              ...
              <Link href="/signin" className="text-[14px] font-medium text-white/95" onClick={() => setIsOpen(false)}>Signin</Link>
            </div>
          </div>
          ...
          <div className="bg-[#F6F7F7] rounded-xl overflow-hidden mb-8">
            <ul className="flex flex-col">
              {categories.map((cat) => (
                <li key={cat.name} className="border-b border-[#E8EAEB] last:border-0 mx-4">
                  <Link 
                    href={`/category/${cat.name.toLowerCase().replace(/ & /g, '-').replace(/ /g, '-')}`} 
                    className={`flex items-center justify-between py-3.5 transition-colors ${cat.active ? 'text-primary font-medium' : 'text-gray-700'}`}
                    onClick={() => setIsOpen(false)}
                  >
                    <span className="text-[15px]">{cat.name}</span>
                    {cat.hasIcon && (
                      <ChevronRight size={18} strokeWidth={cat.active ? 2 : 1.5} className={cat.active ? 'text-primary' : 'text-gray-400'} />
                    )}
                  </Link>
                </li>
              ))}
          ...
          <div className="mb-4">
            <div className="inline-block relative">
              ...
              <div className="absolute -bottom-1.5 left-0 w-8 h-[2px] bg-primary"></div>
            </div>
          </div>
